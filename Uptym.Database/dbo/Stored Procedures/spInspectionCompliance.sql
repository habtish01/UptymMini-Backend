...
CREATE PROCEDURE [dbo].[spInspectionCompliance]
@year int=0,
@FacilityId int =0,
@OwnerId int =0
AS
...
(COUNT(CASE WHEN [Maintenance].[WorkOrderHeader].[IsClosed]=1 THEN 1 END)) AS NumberOfCompletedIMs,
(COUNT(Maintenance.WorkOrderHeader.Id)) AS NumberOfScheduledIMs
FROM
...
WHERE 
[Maintenance].[WorkOrderHeader].WorkorderTypeId=6 
